...
export default function FieldMappingStep({
  ...
  const getConfidenceColor = (confidence: number) => {
    if (confidence >= 90)
      return "text-[#008D0E] bg-[#B0F0C2] border-1 border-[#B0F0C2]";
    if (confidence >= 70)
      return "text-[#8C5E04] bg-[#FFF8DD] border-1 border-[#F6CD7E] rounded";
      ...
      <div className="flex-shrink-0 p-4 pb-0">
        <div className="rounded-lg p-2">
          ...
          <p className="text-[#68818C] text-[17px] font-normal mb-6 leading-[120%] tracking-[0%]">
            ...
            mappings. Review the results and proceed to the next step to make
            any adjustments.
          </p>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        <div className="space-y-3">
          {mappings.map((mapping, index) => (
            ...
            >
              <div
                className={`px-2 py-1 text-[12px] font-medium rounded-[8px] leading-[136%] tracking-[-0.41px] min-w-[25px] h-[16px] flex items-center justify-center ${getConfidenceColor(
                  mapping.confidence
                )}`}
              >
                {mapping.confidence}%
